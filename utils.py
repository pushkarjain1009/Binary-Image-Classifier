...
class Visualize:
    ...
    def plot(self, arr, y_label, title):

        x = len(arr)
        plt.plot(range(x),arr)
        plt.xlabel('Epoch')
        plt.ylabel(y_label)
        plt.title(title)
        plt.show()
    ...
    def plot_value_array(self, i, predictions_array, true_label):
        ...
        plt.xticks([])
        plt.yticks([])
        thisplot = plt.bar(range(2), prediction_array, color="#777777")
        plt.ylim([0,1])
    ...
    def test_plot(self, X_test, y_test, y_pred, predictions_array, classes):
        ...
        for i in range(num_images):
            ...
            self.plot_image(i, y_pred, y_test, X_test, classes)
            plt.subplot(num_rows, 2*num_cols, 2*i+2)
            self.plot_value_array(i,y_pred, y_test)
